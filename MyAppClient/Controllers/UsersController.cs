...
namespace Myapp.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        // POST: api/v1/users
        [HttpPost]
        [Authorize(Roles = "Admin")] // Only allow Admin to add users
        public async Task<ActionResult<User>> PostUser(User user)
        {
            _context.Users.Add(user);
            await _context.SaveChangesAsync();
        ...
        // PUT: api/v1/users/{id}
        [HttpPut("{id}")]
        [Authorize(Roles = "Admin")] // Only allow Admin to update users
        public async Task<IActionResult> PutUser(int id, [FromBody] UserViewModel userViewModel)
        {
            if (userViewModel == null || id != userViewModel.Id)
            {
            ...
            existingUser.EmailId = userViewModel.EmailId;
        ...
        // DELETE: api/v1/users/{id}
        [HttpDelete("{id}")]
        [Authorize(Roles = "Admin")] // Only allow Admin to delete users
        public async Task<IActionResult> DeleteUser(int id)
        {
            var user = await _context.Users.FindAsync(id);
            if (user == null)
